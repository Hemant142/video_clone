      ...
      <div className="border border-gray-300 p-4 rounded-lg transition duration-300 ease-in-out transform hover:scale-105">
       <Link to={`/singleplayer/${video.postId}`}>
        ...
        <div className="mt-2">
          ...
          <p className="text-gray-600">{expend?video.submission.description:`${video.submission.description.substring(0,50)}...`} 
          {!expend?(<button className='font-bold ' onClick={()=>setExpend(!expend)}>Read More</button>):(<button className='font-bold ' onClick={()=>setExpend(!expend)}>Read Less</button>)}
          </p>
        </div>
